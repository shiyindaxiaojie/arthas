...
JAVA_OPTS="${JAVA_OPTS} -XX:MetaspaceSize=${METASPACE_SIZE:-128M} -XX:MaxMetaspaceSize=${MAX_METASPACE_SIZE:-128M}"
...
if [[ "${NACOS_CONFIG_ENABLED}" == "true" ]]; then
  echo "Connect to nacos config server address: ${NACOS_CONFIG_SERVER_ADDR}."
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.enabled=true"
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.server-addr=${NACOS_CONFIG_SERVER_ADDR}"
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.namespace=${NACOS_CONFIG_NAMESPACE}"
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.group=${NACOS_CONFIG_GROUP}"
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.username=${NACOS_CONFIG_USERNAME}"
  JAVA_OPTS="${JAVA_OPTS} -Dspring.cloud.nacos.config.password=${NACOS_CONFIG_PASSWORD}"
fi

exec java $JAVA_OPTS -noverify -Djava.security.egd=file:/dev/./urandom -jar ${HOME}/app.jar --spring.config.additional-location=${HOME}/application.properties
